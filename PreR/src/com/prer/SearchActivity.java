...
public class SearchActivity extends Activity {
	...
	protected ArrayList<Procedure> mainProcedureList; // List containing set of all procedures
	protected ArrayList<Procedure> filteredProcedures; // Procedures filtered by zip code
	protected ArrayList<String> filteredResults; // Procedures filtered by zip code and by name
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String[] defaultProcedures = getResources().getStringArray( 
				R.array.procedure_list); // Gets array of procedure names from resources
		...
		resultsAdapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_list_item_1, filteredResults); // Setup ListView adapter
		results_listView.setAdapter(resultsAdapter); 
	}

	protected void initLayout() {
		setContentView(R.layout.main_search_layout); // Sets layout
		results_listView = (ListView) findViewById(R.id.results_list); // Initializes View fields
		...
		searchButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				String zipcodeText = zipcodeEditText.getText().toString(); // Get entered zipcode
				if (!zipcodeText.isEmpty() && zipcodeText.length() == 5) { 
					...
					for (Procedure procedure : mainProcedureList) {
						if (procedure.zipcode.equals(zipcodeText)) {
							filteredProcedures.add(procedure); // Add all procedures that match
						}
					}

					searchEditText.setVisibility(View.VISIBLE); // Show search field
		...
		searchEditText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				String searchText = searchEditText.getText().toString(); // Get search field text
				filteredResults.clear(); // Clear results
				if (!searchText.isEmpty()) { 
					for (Procedure procedure : filteredProcedures) {
						if (procedure.name.toLowerCase(Locale.getDefault()).contains(
								searchText.toLowerCase(Locale.getDefault()))) {
							filteredResults.add(procedure.name); // Add procedures that match
						}
					}
					if (filteredResults.size() == 0)
						filteredResults.add(getResources().getString(
								R.string.no_results)); // Set default if no results
				}
				resultsAdapter.notifyDataSetChanged(); // Notify adapter that something has changed
			}
